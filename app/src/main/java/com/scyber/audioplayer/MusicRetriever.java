...
import scyber.util.Tree;
import scyber.util.Node;
...
public class MusicRetriever {
  ...
  // the SFiles (songs) we have queried
  List<SFile> sFiles = new ArrayList<SFile>();
  Tree<SFile> tree = new Tree<SFile>();
  ...
  public void prepare() {
    ...
    for (Node<SFile> n : tree.getTraversal().inOrder()) {
      sFiles.add(n.getData());
    }
  ...
  /** Returns a random SFile. If there are no SFiles available, returns null. */
  public SFile getRandomSFile() {
    if (sFiles.size() <= 0) return null;
    return sFiles.get(mRandom.nextInt(sFiles.size()));
  }
}
